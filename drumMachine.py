...
def draw_grid(clicks, beat, actives):
    ...
    kick_drum_text = label_font.render('Kick drum', True, colors[actives[2]])
    screen.blit(kick_drum_text, (30, 230)) 
    crash_cymbal_text = label_font.render('Crash', True, colors[actives[3]])
    ...
    for i in range(beats):
        for j in range(instruments):
            ...
            rect = pygame.draw.rect(screen, color, 
                ...
                ((HEIGHT - 200)//instruments )], 0, 4)
            
            pygame.draw.rect(screen, gold, 
                [i * ((WIDTH - 200)// beats ) + 205, (j * 100), ((WIDTH - 200)// beats), 
...
def draw_save_menu(beat_name, typing):
    ...
    return exit_btn, saving_btn, entry_rectangle
...
def draw_load_menu(index):
    ...
    load_bpm = 0
    pygame.draw.rect(screen, black, [0, 0, WIDTH, HEIGHT])
    menu_txt = label_font.render('LOAD MENU: Select a Beat to Edit.', True, white)
    ...
    screen.blit(delete_txt, ((WIDTH//2) - 485, HEIGHT * .87 + 30))
    screen.blit(menu_txt, (400, 40))
    exit_btn = pygame.draw.rect(screen, gray, [WIDTH - 200, HEIGHT - 100, 180, 90], 0, 5)
    ...
    for beat in range(len(saved_beats)):
        if beat < 10:
            beat_clicked = []
            row_txt = medium_font.render(f'{beat + 1}', True, white)
            screen.blit(row_txt, (200, 100 + beat * 50))
            name_index_start = saved_beats[beat].index('name: ') + 6
            name_index_end = saved_beats[beat].index(', beats:')
            name_txt = medium_font.render(saved_beats[beat][name_index_start:name_index_end], True, white)
            screen.blit(name_txt, (240, 100 + beat * 50))
        if 0 <= index < len(saved_beats) and beat == index:
            beat_index_end = saved_beats[beat].index(', bpm:')
            load_beats = int(saved_beats[beat][name_index_end + 8:beat_index_end])
            bpm_index_end = saved_beats[beat].index(', selected:')
            load_bpm = int(saved_beats[beat][beat_index_end + 6:bpm_index_end])
            load_clicks_string = saved_beats[beat][bpm_index_end + 14: -3]
            load_clicks_rows = list(load_clicks_string.split('], ['))
            for row in range(len(load_clicks_rows)):
                load_clicks_row = (load_clicks_rows[row].split(', '))
                for item in range(len(load_clicks_row)):
                    if load_clicks_row[item] == '1' or load_clicks_row[item] == '-1':
                        load_clicks_row[item] = int(load_clicks_row[item])
                beat_clicked.append(load_clicks_row)
                load_clicked = beat_clicked
    load_info = [load_beats, load_bpm, load_clicked]
...
while run:
    ...
    bpm_text = medium_font.render('Beat Per Minute', True, white)
    ...
    # load menu
    if load_menu:
        exit_button, loading_button, delete_button, loaded_rectangle, load_info = draw_load_menu(index) 
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONUP and not save_menu and not load_menu:
            ...
            # bpm modifier functionality
            if bpm_add_rect.collidepoint(event.pos):
                bpm += 5
            elif bpm_sub_rect.collidepoint(event.pos):
                bpm -= 5
            
            # beat modifier functionality
            ...
            if clear_button.collidepoint(event.pos):
                clicked = [[-1 for _ in range(beats)] for _ in range(instruments)]
           # toggle instrument functionality
